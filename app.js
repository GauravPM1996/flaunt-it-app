...
const App = () => {
  ...
  const listAllRecursive = useCallback(async (storageRef) => {
    ...
    try {
      ...
      for (const itemRef of res.items) {
        ...
        const category =
          pathParts.length > 1 && pathParts[0] === "tattoos"
            ? pathParts[1]
            : "uncategorized";
      ...
      console.warn(
        `Error listing items in ${storageRef.fullPath}: ${error.message}`
      );
    }
    return allMetadata;
  }, []);
  ...
  const getNextImageIndex = useCallback(() => {
    ...
    if (availableIndices.length === 0 && tattooImagesMetadata.length > 0) {
      const newUnseen = new Set(
        Array.from({ length: tattooImagesMetadata.length }, (_, i) => i)
      );
    ...
    unseenImageIndices,
    tattooImagesMetadata,
    likedCategoriesCount,
  ...
  useEffect(() => {
    ...
    if (!firebaseConfig || Object.keys(firebaseConfig).length === 0) {
      const errorMsg =
        "Firebase config is missing or empty. Please provide it.";
      ...
      const authenticate = async () => {
        try {
          ...
          if (currentUserUid) {
            ...
            setIsAuthReady(true);
            console.log(
              "Firebase useEffect: Auth ready. User ID:",
              currentUserUid
            );
          } else {
            ...
            setIsAuthReady(true);
            console.error(
              "Firebase useEffect: Anonymous sign-in resolved but no UID found, using generated UUID. Data storage may fail."
            );
            setFirebaseInitError(
              "Authentication error: User ID not available after sign-in. Data storage may fail."
            );
          }
        } catch (error) {
  ...
  useEffect(() => {
    if (!storage) {
      console.log(
        "Storage not initialized yet, skipping image metadata fetch."
      );
      return;
    }
    ...
    const fetchAllTattooImagesMetadata = async () => {
      ...
      try {
        ...
        if (allMetadata.length === 0) {
          setImagesError(
            "No tattoo images found in the 'tattoos/' folder or its subfolders. Please upload images there."
          );
        }
        ...
        setUnseenImageIndices(
          new Set(Array.from({ length: shuffledMetadata.length }, (_, i) => i))
        );

        console.log(
          "Successfully fetched and shuffled all image metadata:",
          shuffledMetadata.map((m) => m.fullPath)
        );
      } catch (error) {
  ...
  useEffect(() => {
    const loadImage = async () => {
      if (
        tattooImagesMetadata.length > 0 &&
        currentImageIndex < tattooImagesMetadata.length
      ) {
        setCurrentImageLoading(true); // Indicate loading of current image
        setImagesError(null); // Clear previous errors
        try {
          const url = await getDownloadURL(
            tattooImagesMetadata[currentImageIndex].ref
          );
          setCurrentImageUrl(url);
          console.log(
            `Loaded image URL for index ${currentImageIndex}: ${url}`
          );
        } catch (error) {
  ...
  const goToNextImage = useCallback(() => {
    if (timerRunning && tattooImagesMetadata.length > 0) {
      ...
      if (unseenImageIndices.size === 1 && tattooImagesMetadata.length > 0) {
        // If only one left before this action
  ...
  const handleLike = useCallback(() => {
    ...
    setLikedImages((prev) => [
      ...prev,
      tattooImagesMetadata[currentImageIndex]?.fullPath || "unknown_path",
    ]);
    goToNextImage();
  ...
  const handleReset = useCallback(() => {
    ...
    setUnseenImageIndices(
      new Set(Array.from({ length: tattooImagesMetadata.length }, (_, i) => i))
    );
  ...
  const handlePointerMove = useCallback(
    (e) => {
      if (!isDragging || !timerRunning || tattooImagesMetadata.length === 0)
        return;
    ...
    (e) => {
      ...
      if (Math.abs(deltaX) > swipeThreshold) {
        const finalTranslateX =
          deltaX > 0 ? window.innerWidth * 1.5 : -window.innerWidth * 1.5;
        const rotation = deltaX * 0.1;
        setCardCardTransition(
          "transform 0.5s cubic-bezier(0.68, -0.55, 0.265, 1.55)"
        );
        setCardTransform(
          `translateX(${finalTranslateX}px) rotate(${rotation}deg)`
        );
      } else {
  ...
  useEffect(() => {
    ...
    const onTransitionEnd = () => {
      ...
      if (
        ...
        !currentTransform.includes("translateX(0px)")
      ) {
        const deltaX = currentX - startX;
        if (deltaX > 0) {
          handleLike();
  ...
  const handleSubmit = async (e) => {
    ...
    if (valid) {
      console.log("Firebase readiness check before saving:", {
        db: !!db,
        userId: !!userId,
        isAuthReady,
      });
      if (!db || !userId || !isAuthReady) {
        const errorMsg =
          "Firebase connection not ready. Cannot save data. Please check console for details.";
        console.error(errorMsg + " Current states:", {
          db,
          userId,
          isAuthReady,
        });
      ...
      try {
        ...
        console.log(
          "Attempting to save data to Firestore path:",
          userDocRef.path
        );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-700 text-white flex items-center justify-center p-4 font-inter select-none">
      ...
      <div className="bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-sm flex flex-col items-center space-y-6">
        <h1 className="text-3xl font-bold text-center mb-4 text-orange-400">
          Flaunt it
        </h1>
        ...
        <div className="text-xl font-semibold text-gray-300">
          ...
          <span
            className={
              timeLeft <= 10 && timerRunning ? "text-red-500" : "text-green-400"
            }
          >
            {timeLeft}s
          </span>
          ...
          (metadataLoading ? ( // Show loading for image metadata
            ...
            <div className="text-center text-lg space-y-4 text-red-400">
              ...
              <p className="text-sm text-gray-400">
                Please ensure Firebase Storage is enabled and rules allow read
                access to the 'tattoos' folder and its subfolders.
              </p>
            </div>
          ) : tattooImagesMetadata.length === 0 ? ( // No images found after metadata loaded
            <div className="text-center text-lg space-y-4 text-gray-400">
              <p>
                No tattoo images found in the 'tattoos/' folder or its
                subfolders.
              </p>
              <p className="text-sm">
                Please upload images directly to the 'tattoos/' folder or into
                subfolders within it in Firebase Storage.
              </p>
            </div>
          ) : allImagesViewed ? (
            <div className="text-center text-lg space-y-4">
              ...
              <p className="text-gray-400">
                Please submit your details in the popup to continue your
                journey!
              </p>
            </div>
          ) : (
            <>
              ...
              <div
                ...
                ref={cardRef}
                className={`relative w-full pb-[150%] bg-gray-700 rounded-lg overflow-hidden shadow-lg border-2 border-gray-600 ${
                  timerRunning
              ...
              >
                {currentImageLoading ? ( // Show loading spinner for current image
                  ...
                  <img
                    src={
                      currentImageUrl ||
                      "https://placehold.co/400x500/333/FFF?text=Image%0ALoad%0AError"
                    }
            ...
            {!submissionCompleted ? (
              <>
                ...
                <p className="text-gray-300">
                  Want to receive news and be the first to get our professional
                  tattoos?
                </p>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <input
                    ...
                    className={`w-full p-3 rounded-lg bg-gray-700 border text-white placeholder-gray-400 focus:outline-none focus:ring-2 ${
                      emailError
              ...
              <>
                ...
                <p className="text-gray-300">
                  Thank you for connecting with **Flaunt it**! Look forward to
                  exciting updates and exclusive opportunities in the future.
                </p>
              </>
            )}
          </div>
